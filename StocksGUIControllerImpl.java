 ...
 */
public class StocksGUIControllerImpl implements StocksController, Features {
  private StocksGUIView view;
  private StocksModel model;

  /**
   * Constructs a StocksGUIControllerImpl object.
   *
   ...
   *              the user.
   */
  public StocksGUIControllerImpl(StocksGUIView view, StocksModel model) {
    this.view = view;
    this.model = model;
  ...
  @Override
  public void execute() {
    // needed by implementation, unused for this class
  }
  ...
  public void getPortfolioOverview(String selectedItem, String date) {
    ...
    for (int i = 0; i < this.model.getPortfolios().get(index).getStockListSize(); i++) {
      LocalDate d = LocalDate.parse(model.getPortfolios().get(index).getStock(i)
              .getDatePurchased());
